...
import {
  getHermesAttributionDefaults,
  normalizeHermesSignal,
  validateHermesPayload,
...
// Load recent non-archived signals for ALL personas in one query rather than
// one-per-signal. Called once at the start of importHermesPayload and used
// to build a lookup map keyed by personaId.
async function loadRecentSignalsByPersona(personaIds) {
  if (!personaIds.length) return new Map();

  const idList = personaIds.map(sqlString).join(", ");
  const rows = await querySql(`
    SELECT id, persona_id, topic, priority_score, status, validation_id, evidence_urls
    FROM signals
    WHERE persona_id IN (${idList})
      AND status != 'archived'
      AND date(last_seen_at) >= date('now', '-7 day')
  ...
  for (const row of rows) {
    const mapped = mapSignal(row);
    if (!byPersona.has(mapped.personaId)) byPersona.set(mapped.personaId, []);
    byPersona.get(mapped.personaId).push(mapped);
  }
  return byPersona;
}

function findDuplicateInCache(signal, cache) {
  const candidates = cache.get(signal.personaId) || [];
...
async function updateSignal(runId, existing, signal) {
  ...
  `);
  await snapshotSignal(existing.id, runId, signal, {
    ...signal.rawData,
...
export async function importHermesPayload(payload) {
  ...
  try {
    // Normalize all signals up front so we know which personaIds we need.
    for (const persona of payload.personas) {
      for (const rawSignal of persona.signals) {
        const signal = normalizeHermesSignal(
          rawSignal,
          persona.personaId,
          payload.runType,
          payload.generatedAt,
    ...
    // KEY CHANGE: load all recent signals for affected personas in ONE query
    // instead of doing a separate DB round-trip per signal in findDuplicateSignal.
    const personaIds = [...new Set(normalizedSignals.map((s) => s.personaId))];
    const recentByPersona = await loadRecentSignalsByPersona(personaIds);

    // Process signals concurrently — inserts/updates are independent rows so
    // parallel execution is safe. Promise.allSettled lets us collect all results
    // even if one individual signal write fails.
    const signalResults = await Promise.allSettled(
      normalizedSignals.map(async (signal) => {
        const duplicate = findDuplicateInCache(signal, recentByPersona);
        if (duplicate) {
          const signalId = await updateSignal(runId, duplicate, signal);
          return { signalId, wasUpdate: true };
        } else {
          const signalId = await insertSignal(runId, signal);
          // Add to cache so subsequent signals in the same batch don't
          // create duplicates of something we just inserted.
          const cached = recentByPersona.get(signal.personaId) || [];
          cached.unshift({ id: signalId, personaId: signal.personaId, topic: signal.topic });
          recentByPersona.set(signal.personaId, cached);
          return { signalId, wasUpdate: false };
        }
      })
    );
    ...
    for (const result of signalResults) {
      if (result.status === "rejected") {
        console.error("[hermesImport] signal write failed:", result.reason?.message || result.reason);
        continue;
      }
      const { signalId, wasUpdate } = result.value;
    ...
    await execSql(`
      ...
      SET status          = 'completed',
          ...
          cluster_count   = ${new Set(normalizedSignals.map((s) => s.clusterId)).size},
    ...
    await audit("hermes.import.completed", "ingestion_run", runId, {
      imported,
      updated,
      runType: payload.runType,
    });

    // Velocity alerts and validation audit can run concurrently — neither
    // depends on the other's result.
    const postImportTasks = [
      generateVelocityAlerts({ signalIds: [...new Set(importedSignalIds)] }),
    ];

    if (payload.runType === "validation_ping") {
      postImportTasks.push(
        audit("hermes_validation_imported", "ingestion_run", runId, {
          validationId: payload.validationId,
          importedSignalIds,
          provider: attribution.provider,
          model: attribution.model,
    ...
    await Promise.allSettled(postImportTasks);
  } catch (error) {
    await execSql(`
      UPDATE ingestion_runs
      SET status        = 'failed',
  ...
  return {
    ...
    imported,
    updated,
    signalsReceived: normalizedSignals.length,
    importedSignalIds,
  };
}
